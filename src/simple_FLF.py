...
class MSA_block(nn.Module):
    def __init__(self,in_channels,out_channels,use_bn = True,level = 1,**kwargs):
        ...
        self.squeese = nn.Sequential(
            BasicConv(3*out_channels,out_channels,use_bn=use_bn,kernel_size = 1,padding = 0)
        )
...
class Triangle(nn.Module):
    def __init__(self,gray = False,log = None,use_bn = True,training = True):
        ...
        self.FinalConv1 = BasicConv(in_channels=8,out_channels=3,use_bn=True,kernel_size=3,stride=1,padding=1)
